...
class MainWidget(QtGui.QWidget, Ui_MainWidget):
    ...
    def update_values(self, soller_x, soller_z, soller_theta):
        if soller_x is not None:
            if not self.soller_x_pos_txt.hasFocus():
                self.soller_x_pos_txt.setText(str(float(soller_x)))
        if soller_z is not None:
            if not self.soller_z_pos_txt.hasFocus():
                self.soller_z_pos_txt.setText(str(float(soller_z)))
        if soller_theta is not None:
            if not self.soller_theta_pos_txt.hasFocus():
                self.soller_theta_pos_txt.setText(str(float(soller_theta)))
